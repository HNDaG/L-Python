...
def test_create_invalid_total_type():
    with pytest.raises(TypeError):
        resource = inventory.Resource("Parrot", 'Pirates A-Hoy', 10.5, 5)

def test_create_invalid_allocated_type():
    with pytest.raises(TypeError):
        resource = inventory.Resource("name", 'man', 10, 1.5)

def test_create_invalid_total_type():
    with pytest.raises(ValueError):
        resource = inventory.Resource("name", 'man', -10, 0)

@pytest.mark.parametrize('total,allocated', [(10, -5), (10, 20)])
def test_create_invalid_allocated_value(total, allocated):
    with pytest.raises(ValueError):
        inventory.Resource('name', 'man', total, allocated)
